...
function supportsAvx2() {
  ...
  if (process.env.OPENAGENT_HARNESS_FORCE_BASELINE === "1") {
    return false;
  }
...
function getPackageBaseName() {
  try {
    ...
    return packageJson.name || "openagent-harness";
  } catch {
    return "openagent-harness";
  }
}
...
function main() {
  ...
  try {
    ...
    console.error(`\nopenagent-harness: ${error.message}\n`);
    process.exit(1);
  }
  ...
  if (resolvedBinaries.length === 0) {
    console.error(`\nopenagent-harness: Platform binary not installed.`);
  ...
  for (let index = 0; index < resolvedBinaries.length; index += 1) {
    ...
    if (result.error) {
      ...
      console.error(`\nopenagent-harness: Failed to execute binary.`);
