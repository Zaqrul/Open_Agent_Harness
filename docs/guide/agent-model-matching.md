...
OpenAgent Harness assigns each agent a model that matches its _working style_ — like building a team where each person is in the role that fits their personality.
...
Model capabilities are models.dev-backed, with a refreshable cache and capability diagnostics. Use `bunx openagent-harness refresh-model-capabilities` to update the cache, or configure `model_capabilities.auto_refresh_on_start` to refresh at startup.

To see which models your agents will actually use, run `bunx openagent-harness doctor`. This shows effective model resolution based on your current authentication and config.
