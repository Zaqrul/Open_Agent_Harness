...
Complete reference for OpenAgent Harness plugin configuration. During the rename transition, the runtime recognizes both `openagent-harness.json[c]` and legacy `openagent-harness.json[c]` files.
...
1. Project config: `.opencode/openagent-harness.json[c]` or `.opencode/openagent-harness.json[c]`
2. User config (`.jsonc` preferred over `.json`):

| Platform    | Path candidates |
| ----------- | --------------- |
| macOS/Linux | `~/.config/opencode/openagent-harness.json[c]`, `~/.config/opencode/openagent-harness.json[c]` |
| Windows     | `%APPDATA%\opencode\openagent-harness.json[c]`, `%APPDATA%\opencode\openagent-harness.json[c]` |

**Rename compatibility:** The published package and CLI binary remain `openagent-harness`. OpenCode plugin registration prefers `openagent-harness`, while legacy `openagent-harness` entries and config basenames still load during the transition. Config detection checks `openagent-harness` before `openagent-harness`, so if both plugin config basenames exist in the same directory, the legacy `openagent-harness.*` file currently wins.
...
```json
{
  "$schema": "openagent-harness.schema.json"
}
```

Run `bunx openagent-harness install` for guided setup. Run `opencode models` to list available models.
...
Run `bunx openagent-harness doctor --verbose` to see effective model resolution for your config.
...
- Manual refresh is available via `bunx openagent-harness refresh-model-capabilities`.
...
Install the `opencode-antigravity-auth` package for Google Gemini (search the package registry for the current name and docs). Provides multi-account load balancing, dual quota, and variant-based thinking.
