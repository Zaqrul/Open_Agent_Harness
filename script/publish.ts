...
const PACKAGE_NAME = "openagent-harness"
const bump = process.env.BUMP as "major" | "minor" | "patch" | undefined
const versionOverride = process.env.VERSION
const republishMode = process.env.REPUBLISH === "true"
const prepareOnly = process.argv.includes("--prepare-only")

function githubRepoFromEnv(): string | null {
  const r = process.env.GITHUB_REPOSITORY?.trim()
...
console.log("=== Publishing openagent-harness (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `openagent-harness-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  if (!repo) {
    console.log("GITHUB_REPOSITORY not set; skipping contributor notes from gh api")
    return notes
  }

  const team = new Set<string>(["actions-user", "github-actions[bot]"])
  for (const login of process.env.OH_MY_RELEASE_TEAM_LOGINS?.split(",").map((s) => s.trim()).filter(Boolean) ?? []) {
    team.add(login)
  }

  try {
    const compare =
      await $`gh api "/repos/${repo}/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/openagent-harness.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
