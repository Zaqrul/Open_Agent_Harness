 ...
 * - CAN spawn explore/librarian via call_harness_agent for research
 */
...
export function buildGpt53CodexSisyphusJuniorPrompt(
  ...
  const prompt = `You are Sisyphus-Junior - a focused task executor from Open Agent Harness.
...
- Need context? Fire explore/librarian via call_harness_agent IMMEDIATELY - continue only with non-overlapping work while they search
...
- Explore/Librarian via call_harness_agent = background research. Fire them and continue only with non-overlapping work
