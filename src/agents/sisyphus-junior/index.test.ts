...
describe("createSisyphusJuniorAgentWithOverrides", () => {
  ...
  describe("tool safety (task blocked, call_harness_agent allowed)", () => {
    test("task remains blocked, call_harness_agent is allowed via tools format", () => {
      // given
      const override = {
        tools: {
          task: true,
          call_harness_agent: true,
          read: true,
        },
      }
      ...
      if (tools) {
        ...
        // call_harness_agent is NOW ALLOWED for subagents to spawn explore/librarian
        expect(tools.call_harness_agent).toBe(true)
        expect(tools.read).toBe(true)
      }
      if (permission) {
        expect(permission.task).toBe("deny")
        // call_harness_agent is NOW ALLOWED for subagents to spawn explore/librarian
        expect(permission.call_harness_agent).toBe("allow")
      }
    })
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // then - task blocked, but call_harness_agent allowed for explore/librarian spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
