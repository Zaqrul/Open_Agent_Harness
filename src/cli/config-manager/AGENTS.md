...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `openagent-harness` in `.opencode/opencode.json` plugin array |
...
| `generate-omo-config.ts` | Generate `openagent-harness.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `openagent-harness@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/openagent-harness.jsonc` |
