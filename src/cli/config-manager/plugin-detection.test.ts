...
describe("detectCurrentConfig - single package detection", () => {
  ...
  beforeEach(() => {
    ...
    testOmoConfigPath = join(testConfigDir, "openagent-harness.json")
  ...
  it("detects both legacy and canonical plugin entries", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["openagent-harness", "openagent-harness@3.11.0"] }, null, 2) + "\n", "utf-8")
  ...
  it("returns false when plugin not present with similar name", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["openagent-harness-extra"] }, null, 2) + "\n", "utf-8")
  ...
  it("detects OpenCode Go from the existing omo config", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["openagent-harness"] }, null, 2) + "\n", "utf-8")
...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  it("writes canonical plugin entry for new installs", async () => {
    ...
    expect(savedConfig.plugin).toEqual(["openagent-harness"])
  })
  ...
  it("upgrades a version-pinned legacy entry to canonical", async () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["openagent-harness@3.10.0"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["openagent-harness@3.10.0"])
  })

  it("removes stale legacy entry when canonical and legacy entries both exist", async () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["openagent-harness", "openagent-harness"] }, null, 2) + "\n", "utf-8")
  ...
  it("rewrites quoted jsonc plugin field in place", async () => {
    ...
    expect(savedContent.includes("openagent-harness")).toBe(false)
  })
})
