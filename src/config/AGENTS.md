...
27 schema files composing `OpenAgentHarnessConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── openagent-harness-config.ts    # ROOT: OpenAgentHarnessConfigSchema (composes all below)
...
2. Add field to `openagent-harness-config.ts` root schema
