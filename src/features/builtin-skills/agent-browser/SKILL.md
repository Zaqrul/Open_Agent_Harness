...
echo "pass" | agent-browser auth save github --url https://<git-host>/login --username user --password-stdin
...
---
Install: `bun add -g agent-browser && agent-browser install`. Run `agent-browser --help` for all commands. Find upstream docs via the agent-browser package on the npm registry.
