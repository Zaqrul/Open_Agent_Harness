...
git clone <dev-browser-source-url> /tmp/dev-browser-skill

# Copy to skills directory (adjust path as needed)
# For openagent-harness: already bundled
...
git clone <dev-browser-source-url> $env:TEMP\dev-browser-skill
...
1. Download `extension.zip` from the latest release of dev-browser from the source you use (vendor or mirror).
