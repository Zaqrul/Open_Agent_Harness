  ...
  it("derives package name from file URL plugin keys", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          "file:///D:/configs/user-configs/.config/opencode/node_modules/openagent-harness@latest": [
            {
              scope: "user",
              installPath,
              version: "3.10.0",
    ...
    expect(discovered.errors).toHaveLength(0)
    expect(discovered.plugins).toHaveLength(1)
    expect(discovered.plugins[0]?.name).toBe("openagent-harness")
  })
