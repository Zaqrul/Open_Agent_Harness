...
describe("autoMigrateLegacyPluginEntry", () => {
  ...
  describe("#given opencode.json has a bare legacy plugin entry", () => {
    it("#then replaces openagent-harness with openagent-harness", async () => {
      // given
      writeFileSync(
        join(testConfigDir, "opencode.json"),
        JSON.stringify({ plugin: ["openagent-harness"] }, null, 2) + "\n",
      )
      ...
      expect(result.from).toBe("openagent-harness")
      expect(result.to).toBe("openagent-harness")
  ...
  describe("#given opencode.json has a version-pinned legacy entry", () => {
    it("#then preserves the version suffix", async () => {
      ...
      writeFileSync(
        join(testConfigDir, "opencode.json"),
        JSON.stringify({ plugin: ["openagent-harness@3.10.0"] }, null, 2) + "\n",
      )
      ...
      // then
      expect(result.migrated).toBe(true)
      expect(result.from).toBe("openagent-harness@3.10.0")
      expect(result.to).toBe("openagent-harness@3.10.0")
      expect(mockMigrateLegacyPluginEntry).toHaveBeenCalledWith(join(testConfigDir, "opencode.json"))
    })
  })
  ...
  describe("#given both canonical and legacy entries exist", () => {
    it("#then removes legacy entry and keeps canonical", async () => {
      ...
      writeFileSync(
        join(testConfigDir, "opencode.json"),
        JSON.stringify({ plugin: ["openagent-harness", "openagent-harness"] }, null, 2) + "\n",
      )
  ...
  "plugin": ["openagent-harness@latest"]
}
`,
      )
      ...
      // then
      expect(result.migrated).toBe(true)
      expect(result.from).toBe("openagent-harness@latest")
      expect(result.to).toBe("openagent-harness@latest")
      expect(mockMigrateLegacyPluginEntry).toHaveBeenCalledWith(join(testConfigDir, "opencode.jsonc"))
    })
  })
  ...
  describe("#given only canonical entry exists", () => {
    it("#then returns migrated false and leaves file untouched", async () => {
      // given
      const original = JSON.stringify({ plugin: ["openagent-harness"] }, null, 2) + "\n"
