...
async function importFreshCacheModule(): Promise<typeof import("../auto-update-checker/cache")> {
  mock.module("../auto-update-checker/constants", () => ({
    ...
    PACKAGE_NAME: "openagent-harness",
    NPM_REGISTRY_URL: "https://registry.npmjs.org/-/package/openagent-harness/dist-tags",
    NPM_FETCH_TIMEOUT: 5000,
    VERSION_FILE: join(TEST_OPENCODE_CACHE_DIR, "version"),
    INSTALLED_PACKAGE_JSON: join(TEST_OPENCODE_CACHE_DIR, "node_modules", "openagent-harness", "package.json"),
...
function resetTestCache(): void {
  ...
  mkdirSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "openagent-harness"), { recursive: true })
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "package.json"),
    JSON.stringify({ dependencies: { "openagent-harness": "latest", other: "1.0.0" } }, null, 2)
  )
  writeFileSync(
    ...
    JSON.stringify(
      {
        workspaces: {
          "": {
            dependencies: { "openagent-harness": "latest", other: "1.0.0" },
          },
        },
        packages: {
          "openagent-harness": {},
  ...
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "node_modules", "openagent-harness", "package.json"),
...
describe("invalidatePackage", () => {
  ...
  it("invalidates the installed package from the OpenCode cache directory", async () => {
    ...
    expect(existsSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "openagent-harness"))).toBe(false)

    const packageJson = JSON.parse(readFileSync(join(TEST_OPENCODE_CACHE_DIR, "package.json"), "utf-8")) as {
      dependencies?: Record<string, string>
    }
    expect(packageJson.dependencies?.["openagent-harness"]).toBe("latest")
    ...
    expect(bunLock.workspaces?.[""]?.dependencies?.["openagent-harness"]).toBe("latest")
    expect(bunLock.workspaces?.[""]?.dependencies?.other).toBe("1.0.0")
    expect(bunLock.packages?.["openagent-harness"]).toBeUndefined()
