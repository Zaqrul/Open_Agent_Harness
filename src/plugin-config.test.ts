...
function createConfig(config: Partial<OpenAgentHarnessConfig>): OpenAgentHarnessConfig {
  return OpenAgentHarnessConfigSchema.parse(config)
}
...
describe("parseConfigPartially", () => {
  describe("disabled_hooks compatibility", () => {
    ...
    it("should accept unknown disabled_hooks values for forward compatibility", () => {
      const result = OpenAgentHarnessConfigSchema.safeParse({
...
describe("loadPluginConfig", () => {
  it("should only honor mcp_env_allowlist from user config", () => {
    ...
    writeFileSync(
      join(userConfigDir, "openagent-harness.jsonc"),
      JSON.stringify({ mcp_env_allowlist: ["USER_ONLY_TOKEN"] })
    )
    writeFileSync(
      join(projectConfigDir, "openagent-harness.jsonc"),
  ...
  it("should ignore edits to the renamed legacy backup after migration", () => {
    ...
    const projectDir = join(rootDir, "project")
    const projectConfigDir = join(projectDir, ".opencode")
    const legacyConfigPath = join(projectConfigDir, "openagent-harness.jsonc")
    const backupConfigPath = `${legacyConfigPath}.bak`
    const canonicalConfigPath = join(projectConfigDir, "openagent-harness.jsonc")
  ...
  it("should still load config from legacy path when migration fails", () => {
    ...
    const projectDir = join(rootDir, "project")
    const projectConfigDir = join(projectDir, ".opencode")
    const legacyConfigPath = join(projectConfigDir, "openagent-harness.json")
