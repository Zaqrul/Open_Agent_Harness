...
import { OpenAgentHarnessConfigSchema, type OpenAgentHarnessConfig } from "./config";
import {
  log,
  deepMerge,
...
import { CONFIG_BASENAME, isLegacyConfigFilename } from "./shared/plugin-identity";
...
export function parseConfigPartially(
...
): OpenAgentHarnessConfig | null {
  const fullResult = OpenAgentHarnessConfigSchema.safeParse(rawConfig);
  ...
  for (const key of Object.keys(rawConfig)) {
    ...
    const sectionResult = OpenAgentHarnessConfigSchema.safeParse({ [key]: rawConfig[key] });
  ...
  return partialConfig as OpenAgentHarnessConfig;
}
...
export function loadConfigFromPath(
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      const result = OpenAgentHarnessConfigSchema.safeParse(rawConfig);
...
export function mergeConfigs(
  base: OpenAgentHarnessConfig,
  override: OpenAgentHarnessConfig
): OpenAgentHarnessConfig {
  return {
    ...base,
    ...override,
...
export function loadPluginConfig(
  ...
  if (userDetected.format !== "none" && isLegacyConfigFilename(path.basename(userDetected.path))) {
  ...
  if (projectDetected.format !== "none" && isLegacyConfigFilename(path.basename(projectDetected.path))) {
  ...
  let config: OpenAgentHarnessConfig =
    userConfig ?? OpenAgentHarnessConfigSchema.parse({});
