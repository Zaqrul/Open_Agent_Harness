...
describe("detectPluginConfigFile", () => {
  ...
  test("prefers openagent-harness over openagent-harness when both jsonc files exist", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "openagent-harness.jsonc"), "{}")
    writeFileSync(join(testDir, "openagent-harness.jsonc"), "{}")
    ...
    // then
    expect(result.format).toBe("jsonc")
    expect(result.path).toBe(join(testDir, "openagent-harness.jsonc"))
    expect(result.legacyPath).toBe(join(testDir, "openagent-harness.jsonc"))

    rmSync(testDir, { recursive: true, force: true })
  })

  test("falls back to openagent-harness when openagent-harness doesn't exist", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "openagent-harness.jsonc"), "{}")
  ...
  test("loads openagent-harness.json before openagent-harness.json when no jsonc exists", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "openagent-harness.json"), "{}")
    writeFileSync(join(testDir, "openagent-harness.json"), "{}")
    ...
    // then
    expect(result.format).toBe("json")
    expect(result.path).toBe(join(testDir, "openagent-harness.json"))
    expect(result.legacyPath).toBe(join(testDir, "openagent-harness.json"))
  ...
  test("returns none when no config files exist", () => {
    ...
    // then
    expect(result.format).toBe("none")
    expect(result.path).toBe(join(emptyDir, "openagent-harness.json"))
  ...
  test("loads openagent-harness when only canonical jsonc exists", () => {
    // given
    if (!existsSync(testDir)) mkdirSync(testDir, { recursive: true })
    writeFileSync(join(testDir, "openagent-harness.jsonc"), "{}")
