...
export function logLegacyPluginStartupWarning(deps: LogLegacyPluginStartupWarningDeps = {}): void {
  ...
  logFn("[OpenAgentHarnessPlugin] Legacy plugin entry detected in OpenCode config", {
  ...
  console.warn(
    `[openagent-harness] WARNING: Your opencode.json uses the legacy package name "${LEGACY_PLUGIN_NAME}".`
  ...
  if (migrated) {
    console.warn(`[openagent-harness] Auto-migrated opencode.json: ${result.legacyEntries.join(", ")} -> ${suggestedEntries.join(", ")}`)
  } else {
    console.warn(
      `[openagent-harness] Could not auto-migrate. Please manually update your opencode.json:`
