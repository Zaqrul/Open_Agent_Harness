...
describe("migrateLegacyPluginEntry", () => {
  ...
  describe("#given opencode.json contains oh-my-opencode plugin entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces oh-my-opencode with openagent-harness", async () => {
        ...
        expect(content).toContain("openagent-harness@latest")
  ...
  describe("#given opencode.json contains bare oh-my-opencode entry", () => {
    describe("#when migrating the config", () => {
      it("#then replaces with openagent-harness", async () => {
        const configPath = join(testDir, "opencode.json")
        ...
        expect(result).toBe(true)
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain('"openagent-harness"')
        expect(content).not.toContain("oh-my-opencode")
      })
    })
  })
    ...
    describe("#when migrating the config", () => {
      it("#then keeps the original config untouched and writes the migrated content to a sibling temp file", async () => {
        ...
        try {
          ...
          expect(readFileSync(tempPath, "utf-8")).toContain("openagent-harness@latest")
  ...
  describe("#given opencode.json contains pinned oh-my-opencode version", () => {
    describe("#when migrating the config", () => {
      it("#then preserves the version pin", async () => {
        ...
        expect(result).toBe(true)
        const content = readFileSync(configPath, "utf-8")
        expect(content).toContain("openagent-harness@3.11.0")
      })
    })
  })

  describe("#given opencode.json already uses openagent-harness", () => {
    describe("#when checking for migration", () => {
      it("#then returns false and does not modify the file", async () => {
        const configPath = join(testDir, "opencode.json")
        const original = JSON.stringify({ plugin: ["openagent-harness@latest"] }, null, 2)
  ...
  describe("#given plugin entries contain both canonical and legacy values", () => {
    describe("#when migrating the config", () => {
      it("#then removes the legacy entry instead of duplicating the canonical one", async () => {
        const configPath = join(testDir, "opencode.json")
        writeFileSync(configPath, JSON.stringify({ plugin: ["openagent-harness", "oh-my-opencode"] }, null, 2))
        const { migrateLegacyPluginEntry } = await importFreshMigrationModule()
        ...
        expect(saved.plugin).toEqual(["openagent-harness"])
      })
    })
  })
...
`,
    ...
    "openagent-harness@latest"
  ]`)
      })
    })
  })

  describe("#given opencode.json contains oh-my-openagent (prior rename)", () => {
    it("#then migrates to openagent-harness", async () => {
      const configPath = join(testDir, "opencode.json")
      writeFileSync(configPath, JSON.stringify({ plugin: ["oh-my-openagent@latest"] }, null, 2))
      const { migrateLegacyPluginEntry } = await importFreshMigrationModule()
      expect(migrateLegacyPluginEntry(configPath)).toBe(true)
      expect(readFileSync(configPath, "utf-8")).toContain("openagent-harness@latest")
      expect(readFileSync(configPath, "utf-8")).not.toContain("oh-my-openagent")
    })
  })
