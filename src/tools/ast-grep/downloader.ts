...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[openagent-harness] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  const releaseRoot = process.env.AST_GREP_RELEASE_ROOT_URL?.trim()?.replace(/\/$/, "")
  if (!releaseRoot) {
    log(`[openagent-harness] AST_GREP_RELEASE_ROOT_URL not set; skipping ast-grep binary download`)
    return null
  }
  const downloadUrl = `${releaseRoot}/releases/download/${version}/${assetName}`

  log(`[openagent-harness] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[openagent-harness] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[openagent-harness] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
