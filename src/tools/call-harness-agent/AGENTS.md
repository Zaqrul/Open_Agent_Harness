# src/tools/call-harness-agent/ — Direct Agent Invocation Tool

**Generated:** 2026-04-05

## OVERVIEW

23 files. The `call_harness_agent` tool — direct invocation of named agents (explore, librarian only). Distinct from `delegate-task`: no category system, no skill loading, no model selection. Fixed agent set, same execution modes (background/sync).

## DISTINCTION FROM delegate-task

| Aspect | `call_harness_agent` | `delegate-task` (`task`) |
|--------|-----------------|--------------------------|
...
| File | Purpose |
|------|---------|
| `tools.ts` | `createCallHarnessAgent()` factory — validates agent, routes to executor |
...
| `types.ts` | `CallHarnessAgentArgs`, `AllowedAgentType`, `ToolContextWithMetadata` |
