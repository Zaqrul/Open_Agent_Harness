...
const { createCallHarnessAgent } = require("./tools")

describe("createCallHarnessAgent", () => {
  ...
  test("should reject agent in disabled_agents list", async () => {
    //#given
    const toolDef = createCallHarnessAgent(mockCtx, mockBackgroundManager, ["explore"])
  ...
  test("should allow agent not in disabled_agents list", async () => {
    //#given
    const toolDef = createCallHarnessAgent(mockCtx, mockBackgroundManager, ["librarian"])
    const executeFunc = toolDef.execute as Function
  ...
  test("should allow all agents when disabled_agents is empty", async () => {
    //#given
    const toolDef = createCallHarnessAgent(mockCtx, mockBackgroundManager, [])
    const executeFunc = toolDef.execute as Function
  ...
  test("uses agent override fallback_models when launching background subagent", async () => {
    ...
    const toolDef = createCallHarnessAgent(
      mockCtx,
      managerWithLaunch,
      [],
      {
