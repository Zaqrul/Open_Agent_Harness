...
import { ALLOWED_AGENTS, CALL_HARNESS_AGENT_DESCRIPTION } from "./constants"
import type { AllowedAgentType, CallHarnessAgentArgs, ToolContextWithMetadata } from "./types"
...
function resolveModelAndFallbackChain(args: {
  ...
  if (agentOverride?.model) {
    ...
    if (normalized) {
      ...
      log("[call_harness_agent] Resolved model override from agent config", {
        agent: subagentType,
    ...
    if (normalized) {
      ...
      log("[call_harness_agent] Resolved model override from agent category", {
        agent: subagentType,
...
export function createCallHarnessAgent(
  ctx: PluginInput,
  ...
  const description = CALL_HARNESS_AGENT_DESCRIPTION.replace("{agents}", agentDescriptions)
  ...
  return tool({
    ...
    async execute(args: CallHarnessAgentArgs, toolContext) {
      const toolCtx = toolContext as ToolContextWithMetadata
      log(`[call_harness_agent] Starting with agent: ${args.subagent_type}, background: ${args.run_in_background}`)
      ...
      if (disabledAgents.some((disabled) => disabled.toLowerCase() === normalizedAgent)) {
        return `Error: Agent "${normalizedAgent}" is disabled via disabled_agents configuration. Remove it from disabled_agents in your openagent-harness.json to use it.`
      }
