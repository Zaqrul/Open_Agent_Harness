...
bunDescribe("sendSyncPrompt", () => {
  ...
  bunTest("applies agent tool restrictions for explore agent", async () => {
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.tools.call_harness_agent).toBe(false)
  })
  ...
  bunTest("does not restrict call_harness_agent for sisyphus agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.tools.call_harness_agent).toBe(true)
  })
