...
export async function resolveGrepCliWithAutoInstall(): Promise<ResolvedCli> {
  ...
  try {
    ...
    if (current.backend === "grep") {
      log("[openagent-harness] Failed to auto-install ripgrep. Falling back to GNU grep.", {
        error: error instanceof Error ? error.message : String(error),
        grep_path: current.path,
      })
    } else {
      log("[openagent-harness] Failed to auto-install ripgrep and GNU grep was not found.", {
        error: error instanceof Error ? error.message : String(error),
      })
    }
