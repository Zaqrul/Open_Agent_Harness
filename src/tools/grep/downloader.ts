...
function getInstallDir(): string {
  ...
  return join(homeDir, ".cache", "openagent-harness", "bin")
}
...
export async function downloadAndInstallRipgrep(): Promise<string> {
  ...
  const releaseRoot = process.env.RIPGREP_RELEASE_ROOT_URL?.trim()?.replace(/\/$/, "")
  if (!releaseRoot) {
    throw new Error(
      "RIPGREP_RELEASE_ROOT_URL is not set. Set it to the release root (the URL prefix before /releases/download/...) for your ripgrep mirror, or install `rg` on your PATH.",
    )
  }

  const filename = `ripgrep-${RG_VERSION}-${config.platform}.${config.extension}`
  const url = `${releaseRoot}/releases/download/${RG_VERSION}/${filename}`
